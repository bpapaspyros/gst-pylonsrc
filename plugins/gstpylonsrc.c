...
#define PYLONC_CHECK_ERROR(obj, res)                                                                                       \
    if (res != GENAPI_E_OK) {                                                                                              \
        char* errMsg;                                                                                                      \
        size_t length;                                                                                                     \
        ...
        GST_CAT_LEVEL_LOG(GST_CAT_DEFAULT, GST_LEVEL_NONE, obj, "PylonC error: %s (%#08x).\n", errMsg, (unsigned int)res); \
        free(errMsg);                                                                                                      \
        GenApiGetLastErrorDetail(NULL, &length);                                                                           \
        errMsg = (char*)malloc(length);                                                                                    \
        GenApiGetLastErrorDetail(errMsg, &length);                                                                         \
        GST_CAT_LEVEL_LOG(GST_CAT_DEFAULT, GST_LEVEL_NONE, obj, "PylonC error: %s\n", errMsg);                             \
        free(errMsg);                                                                                                      \
        goto error;                                                                                                        \
    }
...
/* pad templates */
static GstStaticPadTemplate gst_pylonsrc_src_template = GST_STATIC_PAD_TEMPLATE("src",
    GST_PAD_SRC,
    GST_PAD_ALWAYS,
    GST_STATIC_CAPS_ANY);
...
void gst_pylonsrc_set_property(GObject* object, guint property_id,
...
void gst_pylonsrc_get_property(GObject* object, guint property_id,
...
void gst_pylonsrc_dispose(GObject* object)
{
    GstPylonsrc* pylonsrc = GST_PYLONSRC(object);
...
void gst_pylonsrc_finalize(GObject* object)
{
    GstPylonsrc* pylonsrc = GST_PYLONSRC(object);
